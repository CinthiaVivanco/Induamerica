$(document).ready(function(){
    ...
    $('#buscarnotacredito').on('click', function(event){
        ...
        $.ajax({
            type	: 	"POST",
            url		: 	carpeta+"/ajax-listado-de-notacredito-entrefechas",
            data	: 	{
            				_token	: _token,
            				finicio : finicio,
            				ffin 	: ffin
    ...
    $('#enviarnotacredito').on('click', function(event){
    	event.preventDefault();
    	...
    	data = dataenviar();
    	if(data.length<=0){alerterrorajax('Seleccione por lo menos una nota de credito'); return false;}
    	var datastring = JSON.stringify(data);
    	$('#notacredito').val(datastring);
    	abrircargando();
    	$( "#formnotacredito" ).submit();
    });




	$(".listatablanotacredito").on('click','label', function() {
